...
TEST_CASE("StackAllocator", "[Memory::StackAllocator]") {
  unsigned int memorySize = 1024;
  void* memory = malloc(memorySize * sizeof(u_int8_t));
  StackAllocator a(memory, memorySize * sizeof(u_int8_t));
  SECTION("Single Variable Allocation") {
    ...
    *i = 0;
    ...
    a.Dealocate(*j);
    //REQUIRE(a.GetUsedMemory() == sizeof(int));


    size_t numberInts = static_cast<size_t>(memorySize * sizeof(u_int8_t)) / sizeof(int) - 1;
